...
test('GetPeriodo (sucesso)', () => {
    return expect(PeriodoController.getPeriodo(3)).resolves.toEqual([{id: 3, ano: 2021, semestre: 1}]);
});

test('GetTodosPeriodos (sucesso)', () => {
    return expect(PeriodoController.getTodosPeriodos()).resolves.toEqual([{id: 1, ano: 2020, semestre: 1},{id: 2, ano: 2020, semestre: 2},{id: 3, ano: 2021, semestre: 1}]);
});
...
test('UpdatePeriodo (sucesso)', () => {
    const p = new Promise((resolve, reject) => resolve([{mid: 1}]));
